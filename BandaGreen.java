    ...
    public static BufferedImage apply(BufferedImage image) throws IOException {
        ...
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                // Pegando e mantendo a informação do azul e trocando vermelho e azul pra 0
                Color c = new Color(image.getRGB(x, y));
                int green = c.getGreen();
                Color newCor = new Color(0, green, 0);
                image.setRGB(x, y, newCor.getRGB());
            }
        }
        //Salvando imagem se deu certo :)
        if (ImageIO.write(image,"png", new File("C:\\Users\\cauec\\Pictures\\Filtros\\filtroVerde.png")))
